...
一个基于企业微信智能机器人和 WebSocket 长连接的虚拟员工“牛来”在工作时间内随机触发喊话事件（内容默认“妈妈”，`SCREAM_CONTENT` 可配，每个群聊每天至少保底触发一次），收到剥离开头 @提及 后仍包含停止关键词（默认“牛来”，`STOP_KEYWORD` 可配）的文本后停止、回复完成消息（默认 🐮，可配置图片），并进入 2 小时冷却。每个群聊的触发、停止与冷却相互独立，一个群的停止指令不影响其他群
...
`TARGET_CHAT_ID` 支持使用逗号分隔配置多个会话。运行期间自动发现的新群聊会增量加入目标列表并回写 `.env`，已存在的 ID 不会重复添加；修改启动配置后请重启进程。
